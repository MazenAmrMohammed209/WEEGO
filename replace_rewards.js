  ...
  useEffect(() => {
    async function fetchRewardsData() {
      ...
      try {
        ...
        // Ensure both queries run
        const levelsPromise = supabase
          .from("reward_levels")
          .select("*")
          .order("points_required", { ascending: true });

        let pointsPromise: Promise<any> = Promise.resolve({ data: null, error: null });
        ...
        const [levelsRes, pointsRes] = await Promise.all([levelsPromise, pointsPromise]);

        if (levelsRes.error) {
          console.error("Reward levels fetch error:", levelsRes.error);
        }
        ...
        if (user?.email) {
          if (pointsRes.error && pointsRes.error.code !== "PGRST116") {
            console.error("Loyalty fetch error:", pointsRes.error);
          } else if (pointsRes.data) {
            userTotalPoints = pointsRes.data.total_points || 0;
          }
        }
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-3 mt-auto pt-4 border-t border-border/50">
                    ...
                    {lvl.cost_on_wego && (
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-bold text-foreground/60">Cost</span>
                        <span className="font-semibold text-foreground bg-foreground/[0.04] px-2 py-1 rounded-md">{lvl.cost_on_wego}</span>
                      </div>
                    )}
                    {lvl.eligibility && (
                      <div className="flex items-center justify-between text-sm">
...
fs.writeFileSync('src/app/rewards/page.tsx', content);
